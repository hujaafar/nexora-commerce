...
The production initial bundle is approximately **536 KB raw / 109 KB estimated transferred**, within the existing 540 KB warning and 600 KB error budgets. No animation library or production dependency was added. The ScrollCraft adapter accepts a per-instance motion preference and re-reads the device preference on each mount.

Phones use ordinary document flow with short scroll-linked image transforms. The desktop engine is not mounted at 800px or below. Mobile updates run outside Angular and are coalesced into one animation frame per scroll event. Route changes release the engine, observers and event handlers.

### Scroll behavior

Scroll effects are always on, as requested by the project owner. The storefront has no motion selector and does not read the former stored preference or `motion` query parameter. The document opts into full effects from its initial HTML, and the runtime mounts with full motion even when the device requests reduced motion. Windows and browser preferences are not modified.

Phones retain their compact composition and short image movement without desktop pinning. The [Firefox and Chrome measurements](evidence/motion-preference-browsers.json) record visible image movement on an ordinary `/products` visit. Firefox was tested using wheel input with reduced motion enabled; movement remained active after reloading with the former Off preference saved.
...
- All 32 Angular unit tests passed, including service-state behavior, responsive motion setup, and disposal.
- Seven storefront browser tests passed: desktop painted movement and pointer tilt, compact screens, gallery navigation, automatic animation with a reduced-motion device preference, and ignoring the former Off setting across refreshes. Earlier route sweeps cover 13 route/role combinations at 1440, 390 and 360 pixels.
...
- Desktop ScrollCraft contact sheets sampled both pinned acts at intermediate positions and reported no dead scroll. The phone harness uses an application-readiness selector because compact screens use a separate lightweight motion path. This changes harness startup only, not the runtime or measurements.
