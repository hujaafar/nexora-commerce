...
(function (global) {
  ...
  function mount(root, opts) {
    // Nexora adaptation: Angular routes own the runtime's lifetime.
    var disposed = false, listeners = [], frames = new Set(), timers = new Set(), observers = [];
    var originalStyles = Array.from(root.querySelectorAll('[data-sc-act], .sc-stage, [data-sc-in]')).map(function(el) { return [el, el.getAttribute('style')]; });
    ...
    function destroy() {
      if (disposed) return;
      disposed = true;
      listeners.forEach(function(item) { window.removeEventListener(item[0], item[1], item[2]); });
      frames.forEach(function(id) { window.cancelAnimationFrame(id); });
      timers.forEach(function(id) { window.clearTimeout(id); });
      observers.forEach(function(observer) { observer.disconnect(); });
      originalStyles.forEach(function(item) { if (item[1] === null) item[0].removeAttribute('style'); else item[0].setAttribute('style', item[1]); });
      global.ScrollCraft.instances = global.ScrollCraft.instances.filter(function(instance) { return instance !== api; });
      if (!global.ScrollCraft.instances.length) document.documentElement.classList.remove('sc-ready');
    }
    ...
    var api = { destroy: destroy, layout: layout, read: read, acts: acts, worlds: worlds, clips: playheads, lerp: LERP };
