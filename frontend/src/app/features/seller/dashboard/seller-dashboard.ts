...
export class SellerDashboard implements OnInit, OnDestroy {
  ...
  protected readonly totalImages = computed(() =>
    this.products().reduce((total, product) => total + product.imageUrls.length, 0),
  );
  ...
  protected chooseImages(event: Event): void {
    ...
    for (const file of files) {
      ...
      if (this.pendingImages().length + this.existingImageUrls().length >= 8) {
  ...
  private createProduct(request: ProductRequest): Observable<Product> {
    return this.productService.create({ ...request, imageUrls: [] }).pipe(
      switchMap((created) =>
  ...
  private updateProduct(productId: string, request: ProductRequest): Observable<Product> {
